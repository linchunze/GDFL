...
def ConvBNLayerconv1(net, from_layer, out_layer, use_bn, use_relu, num_output,
...
def ResBody(net, from_layer, block_name, out2a, out2b, out2c, stride, use_branch1, dilation=1, **bn_param):
  ...
  if use_branch1:
    branch_name = 'branch1'
    ConvBNLayer(net, from_layer, branch_name, use_bn=use_bn, use_relu=False,
  ...
  ConvBNLayer(net, from_layer, branch_name, use_bn=use_bn, use_relu=True,
  ...
  if dilation == 1:
    ConvBNLayer(net, out_name, branch_name, use_bn=use_bn, use_relu=True,
  ...
  ConvBNLayer(net, out_name, branch_name, use_bn=use_bn, use_relu=False,
...
def VGGNetBase(net, from_layer, need_fc=True, fully_conv=False, reduced=False,
        dilated=False, nopool=False, dropout=True, freeze_layers=[], dilate_pool4=False):
    kwargs = {
            'param': [dict(lr_mult=1, decay_mult=1), dict(lr_mult=2, decay_mult=0)],
            'weight_filler': dict(type='xavier'),
    ...
    name = 'pool4'
    net[name] = L.Pooling(net.relu4_3, pool=P.Pooling.MAX, kernel_size=3, stride=1)

    kernel_size = 3
    dilation = False
    #net.conv5_1 = L.Convolution(net[name], num_output=512, pad=1, kernel_size=kernel_size, dilation=dilation, **kwargs)
    #net.relu5_1 = L.ReLU(net.conv5_1, in_place=True)
    #net.conv5_2 = L.Convolution(net.relu5_1, num_output=512, pad=1, kernel_size=kernel_size, dilation=dilation, **kwargs)
    #net.relu5_2 = L.ReLU(net.conv5_2, in_place=True)
    #net.conv5_3 = L.Convolution(net.relu5_2, num_output=512, pad=1, kernel_size=kernel_size, dilation=dilation, **kwargs)
    #net.relu5_3 = L.ReLU(net.conv5_3, in_place=True)
...
def ResNet101Body(net, from_layer, use_pool5=True, use_dilation_conv5=False, **bn_param):
    ...
    scale_prefix = 'scale_'
    scale_postfix = ''
    ConvBNLayerconv1(net, from_layer, 'conv1', use_bn=True, use_relu=True,
...
def CreateMultiBoxHead(net, data_layer="data", num_classes=[], from_layers=[],
    ...
    #print (len(from_layers), len(normalizations))
    #import pdb 
    #pdb.set_trace() 
    if normalizations:
    ...
    for i in range(0, num):
        ...
        name = "{}_bbox_loc{}".format(from_layer, loc_postfix)
        ...
        name = "{}_bbox_conf{}".format(from_layer, conf_postfix)
        ...
        name = "{}_bbox_priorbox".format(from_layer)
